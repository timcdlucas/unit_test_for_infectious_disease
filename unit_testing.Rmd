...
Our aim here however is certainly not to single out a single scientist as this level of code checking is common in epidemiology.
...
Unit testing is a formally defined framework that compares the outputs from code to what the programmer was expecting the code to produce. Unit testing is a framework for testing code that is more principled (@wickham2015r Chapter 7, @osborne2014ten, @wilson2014best).
...
Far a function that computes the square root of a number, we would write a test that runs the function on the number 4 and compares it to the known, correct answer i.e. 2.
...
While there are a number of test functions, the two main functions are `expect_true` and `expect_equal`. 
...
Then if the bug re-emerges during further development it will be caught.
Finally, you can write tests before you write code.
This can be a helpful way to force yourself to plan what some new code or functionality is expected to do: write tests that define the wanted functionality, and keep on programming until all the tests pass.
