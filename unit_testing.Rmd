...
While a number of articles have stated that unit tests should be written (@osborne2014ten, @wilson2014best), we have not yet seen a thorough guide as to how to test the sort of code commonly written by infectious disease biologists.
And while the basic premise of unit testing is simple, there is an art to knowing what aspects of code can and should be tested; we hope this primer will enable researchers to acquire this skill quickly.
...
The definition of unit testing is that it tests code with zero dependencies outside of the test definition.
...
Testing at even higher levels includes system testing (which tests how multiple systems such as software and hardware interact) and acceptance testing (in which end-users, or whoever commissioned the software, test that it meets requirements).
Within the scientific community however, the term unit testing is typically used in a slightly vague way and implies a combination of integration and unit testing.
...
This focus on low level functions is beneficial for a number of reasons. 
First, it allows you to work out the exact expected result of a function call.
Second, if you do find bugs, they are easier to isolate and fix if you are working at these low levels.
Third, code that either calls the low level functions or relies on outputs from them is easier to in turn test and debug.
...
This could certainly be considered integration testing rather than unit testing.
...
Here, we had a binomial event (were the pathogen proportions different between the first and last time step) and an expected frequency of this event (greater than zero).  
This approach is the focus of the literature on testing stochastic code  [@vsevvcikova2006automated; @guderlei2007statistical; @patrick2017toolkit].
If we know a property of the expected distribution of a model outcome (the mean for example) we can run the simulation of number of times and use a standard statistical test to compare the distribution of simulated outputs to the expected distribution.
...
Here we are imagining that the users intent was to run a simulation with three pathogens: 1, 2 and 3.
\newline
...
However, bugs often occur at _special cases_---when the code behaves qualitatively differently at a certain value of a paramete or at certain combinations of parameter values.
...
\newline
\newline
We checked earlier if the `t` argument of `initialisePop()` worked by verifying that the returned population had the correct dimensions  (\@ref(testallargs)).
We can test the `fullSim()` function in the same way (\@ref(edge2)).
However, if we try to run the same test with `t = 1` we get an error before we even get to the test.
\newline
...
> popt1 <- fullSim(t = 2, N = 5, pathogens = 3) 
> expect_equal(dim(popt1), c(2, 5))

> popt2 <- fullSim(t = 1, N = 5, pathogens = 3)
...
In general, requesting a simulation with 1 time step is not epidemiologically unmeaningful; this is not an objectively wrong use of the function. 
The code behaves qualitatively differently for `t = 1` than for `t = 2` because the loop in \@ref(combinesimplefunc) runs from 2 to `t`.
When `t` is 1, the population is initialised with 1 row but the code still tries to run a second iteration and tries to put the results in the second row of the population matrix even though that matrix only has 1 row.
...
We should change the code to handle this special case, and use out new test to check that our new code works.
In \@ref(edge3) we use an if statement to explicitely handle the special case of `t = 1` and our test now passes.
\newline
