...
abstract: |
  Models are one of the cornerstones of infectious disease epidemiology and are increasingly reliant on computers rather than pen-and-paper methods. 
...
<!--
Modelling is an important tool for understanding fundamental biological processes in infectious disease dynamics,, evaluating potential intervention efficacy and forecasting disease burden. 3  At the time of writing, infectious disease modellers are playing a central role on the interpretation of available data on the COVID19 pandemic  to inform policy design and evaluation.  [1–3]. This work builds on a long history of modelling for policy (cite Heesterbeek Science 2015)., and a general understanding of the dynamics of these systems.. 
--->
...
At the time of writing, infectious disease modellers are playing a central role in the interpretation of available data on the COVID19 pandemic to inform policy design and evaluation [@ihme; @imperial; @hellewell2020feasibility].
Similarly, policy on endemic infectious diseases, such duration and frquency of control programs and spatial prioritisation, is also directed by models [@behrend2020modelling]. 
This work builds on a long history of modelling for policy [@heesterbeek2015modeling] and a general understanding of the dynamics of these systems.
...
Outside of epidemiology, a paper that informed austerity policies globally was found to be fatally flawed [@herndon2014does] while in engineering bugs have caused spacecraft, a Mars Climate orbiter and the Mariner 1, to be lost or destroyed [@nasa; @board1999mars].
...
To guard against error, policy-makers now standardly request analyses from multiple modelling groups (as is the case in the UK for COVID-19 [@spim]) to provide scientific robustness and reliability [@den2019guidelines], but clearly more can be done.

Infectious disease modellers are rarely trained as professional programmers. 
However, todays's models are increasingly moving from mean-field ordinary differential equation approximations to individual-based models with complex, data-driven contact processes [@willem2017lessons; @ferguson2006strategies].
These increases in model complexity are accompanied with growing codebases. 
As the mathematical methods depend on numerical solutions rather than analytical pen-and-paper methods, it becomes increasingly difficult to tell if a bug is present based on model outputs.
...
At the heart of every unit test is a function output, its known or expected value and a process to compare the two.
...
Setting `replace = TRUE` fixes this (\@ref(correctcode)) and when we plot the proportion of each pathogen (Fig. \@ref(fig:correctplots)) we see the correct behaviour (one pathogen drifting to fixation).
In the subsequent sections we will develop this example as we consider different concepts in unit testing, resulting in well-tested code by the end. 
\newline
...
To ensure the unit tests are evaluating the same code as is used in the actual analysis, the model code needs to be defined once within a function.
This same function is then used for the unit tests as well as part of the larger model codebase.
...
At this stage we have also modified the code so that we can vary the number of pathogens using the `pathogens` argument in the `initialisePop()` function.
\newline
...
We now expect there to be no `NA`s left in the population matrix and every element to be one of "a", "b" or "c".
\newline
...
As above, `initialisePop()` does not throw an error if a vector is supplied to `t`. 
...
If the code that runs before the error is thrown (in this case the `initialisePop()` function) takes a long time, it can be time consuming to work out why the function is not working
It is also a signature of non-robust code that the error is coincidental; a small change in the code might stop the error from occuring.
To improve this code we can add an additional argument check to `initialisePop()`.
...
\newline
```{r, edge2, ref = "edge2", codecap = "initialisePop does not give a population with the correct number of pathogens if N is less than pathogens.", eval = FALSE}
...
Here however, we will decide that this behaviour is actually the behaviour we want.
The test above was still useful to highlight the behaviour.
Now however, our expected output from the function has changed and so we change our test to reflect that; we now expect a population with `N` pathogens.
\newline
