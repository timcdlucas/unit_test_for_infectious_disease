...
There are many aspects of code that are best tested using compact functions.
...
We can test very broad scale properties such as in the test above where the only thing we are testing is that all the values are one of the possible strains.
However, there are many ways some code could pass that test and still be wrong (for example the original, buggy code would pass that test).
...
For example, the function `update_pop` above does a number of things, some of which are stochastic and some of which are deterministic.
Firstly, the function stochastically samples who gets infected by whom.
