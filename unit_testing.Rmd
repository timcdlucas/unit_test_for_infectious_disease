...
Outside of epidemiology, a paper that informed austerity policies globally was found to be fatally flawed [@] while in engineering bugs have caused spacecraft a Mars Climate orbiter and the Mariner 1 spacecraft to be lost or destroyed [@nasa; @board1999mars].
...
Once release, it became clear that the code was not tested [@ferg_github].
...
Consider a multi-pathogen system, with a population of $N$ infected individuals who each get reinfected at every time step.
...
The aim of this model however is not to be a good model of reality, but instead to be a useful example that can be coded very succinctly.
...
_Solution_: `sample()` defaults to sampling without replacement `sample(..., replace = FALSE)`; this means everyone swaps their infection pathogen on a one-to-one basis rather than one-to-many as required by the model.
...
We now expect there to be no `NA`s left in matrix `I` and every element to be one of the three pathogen strings.
\newline
