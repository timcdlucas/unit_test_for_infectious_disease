...
The fix for this particular bug is that the \texttt{sample} function defaults to sampling without replacement.
Setting \texttt{replace = TRUE} fixes the code and when we plot the proportion of each strain (Figure \@ref{fig:correctplots}) we now see the correct behaviour.
