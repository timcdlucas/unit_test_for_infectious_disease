...
pop <- initialisePop(t = 10, N = 2, pathogens = 3) 
expect_equal(length(unique(pop[1,])), 3)
```

Again, the best solution here is to add a check at the beginning of the function.
We now expect that the function should return an error if the user asks for more pathogens than individuals.
\newline
```{r, edge3, ref = "edge3", codecap = "Check that pathogens is not larger than N", eval = FALSE}
...
initialisePop <- function(t, N, pathogens){
  ...
  if(length(pathogens) > 1) stop('pathogens must have length 1')
  if(length(t) > 1) stop('t must have length 1')
  if(pathogens > N) stop('N must be greater than pathogens')
...
expect_error(initialisePop(t = 10, N = 2, pathogens = 3),
             'N must be')
```
