...
author: Tim CD Lucas, Timothy M Pollington, T Déirdre Hollingsworth, et al.
geometry: margin=1in
...
They help guide health policy that can affect millions of people. Yet a single bug could bias results, leading to incorrect conclusions and wrong actions that could cause avoidable harm.
Therefore it is arguably an ethical responsibility that our code is as free of error as possible. Software engineering already provides a solution in _unit testing_, a defensive coding method to avoid bugs in code.

We demonstrate through simple examples how unit testing can handle the particular quirks of infectious disease models and encourage modellers from infectious diseases to ecology to try implementing unit testing in their next script.
...
They may guide policy for the control of endemic disease or for fast-moving epidemics. 
For instance models are currently being used to directly advise governments on their response to the COVID-19 pandemic [@ihme; @imperial; @hellewell2020feasibility] and [an_ntd_paper_that_definitely_influenced_policy]. 
...
These increases in model complexity are accompanied with growing code bases. The mathematical methods themselves use numerical solution as opposed to analytical pen-and-paper methods. Soon enough it becomes more difficult to tell that a bug lies within based on the results that come out.
...
At the heart of every unit test is a function output, its known/expected output and comparing the two.
For a function that computes the square root $\sqrt{x}$ (`sqrt(x)` in $\texttt{R}), we would write a test that runs the function call on the number 4 i.e. `sqrt(x = 4)` and compares it to the known, correct answer i.e. 2. Often our function arguments will cover an infinite range of possibilities but we cannot exhaustively check them all. Instead we devise tests that cover particular scenarios or 'corner cases': what do we want our function to do if it is given a negative number e.g. `sqrt(-1)`, or a vector argument containing strings or missing values e.g. `sqrt(c(4,"melon",NA))`?

In $\texttt{R}$ [@R], the \texttt{testthat} package [@testthat], provides a simple interface for testing.
...
If a change in new code causes the older tests to break, this points to the introduction of an error in the new code, or the older code could not generalise to the adapted environment.
...
This can be a helpful way to plan functionality: write tests that define the desired functionality, and continue programming so long as all tests keep passing.
...
Certainly this toy model is naive in that it is governed my mass-action principles, ignoring contact and spatial dynamics. Nevertheless it will serve its purpose. Here is our first attempt at implementing this model.
...
# Initialise the population @ t = 1 with a fixed configuration
I[1,] <- rep(x = c("a", "b", "c"), length.out = N)

# At each time step, everyone is re-infected by someone from the previous time step.
for(t in seq(2, t)){
...
However, if we look at the proportion of each strain through time (not shown here) we will quickly notice there must be a bug hiding.
...
This function is then used in both as part of the model code base and in the unit test itself.
...
updatePop <- function(x, i, N){
  x[i,] <- sample(x = x[i-1,], size = N, replace = TRUE)
  return(x)
}
```
...
The first two arguments `(t, N, ...)` directly change the expected dimension of matrix `I`, so we test that.
...
expect_true(all(is.na(pop1[-1,]))) # expect all except the first row are NAs
expect_true(all(c("a", "b", "c") %in% pop[1,])) # expect all 3 strains @ t = 1

pop2 <- updatePop(pop, i = 2, N = 12)
expect_true(all(!is.na(pop2[1:2,]))) # after update expect 1st & 2nd row not to have NAs
```

### Combine simple functions and test the higher-level functions {-#combine}

In the end the entire model runs when functions work together and here we need to test these connections. This can be achieved by nesting functions together, or defining functions at a higher level.
...
We also now expect every element of $I_{it}$ to be one of the three strain strings.

```{r, testnn, results = 'hide'}
pop <- fullSim(t = 12, N = 20)
expect_true(!any(is.na(pop))) # expect no NAs
expect_true(all(pop %in% c("a", "b", "c"))) # expect all 3 strains contained within
```

## Testing stochastic code

A challenges for testing particular to infectious disease modelling is the use of stochastic simulationss.
...
```{r, refactorstoch}
sampleEvents <- function(N){
  sample(x = N, size = N, replace = TRUE)
}

newInfectionStatus <- function(old_i, events){
  old_i[events]
}

addInfectionStatus <- function(x, new_i, i){
  x[i,] <- new_i
}

updatePop <- function(x, N, i){
  events <- sampleEvents(N)
  new_i <- newInfectionStatus(x[i-1,], events)
  x[i,] <- addInfectionStatus(x, new_i, i)
  return(x)
}
...
Again focussing on very simple parameters, there are some cases where the code is stochastic, but has a very small number of possible outputs. 
...
### Use very large samples for the stochastic part {-#largesmaples}
...
This typically includes either using large sample sizes or large numbers of runs of the stochastic code.
...
The probability of a pair of people not being infected by the same person is $1-\frac{1}{P}$ (given that the first person is infected by person $a$, the probability that the second person is infected by $a$ is just $1/P$).
For the test to fail, every pair would have to not be infected by the same person giving $\big(1-\frac{1}{P}\big) ^ {P \choose 2}$.
...
set.seed(20200406)
P <- 1e3
events <- sample_events(P)
expect_true(any(duplicated(events)))
...
For example, consider a function `divide(x, y)` that divides `x` by `y`. 
If we test that `2 * divide(10, 2) == 10` this suggests that the function works for nearly all values of division.
...
Similarly, earlier we tested whether the `strain` argument of `initialise_pop` worked by checking whether the returned population had the correct number of strains.
...
Both of these cases are dangerous or annoying and can be difficult to debug after the fact.
A common case to check, that is common in infectious disease modelling, is testing whether code handles data with NAs in correctly. 
Code will often be written with a clean, complete dataset. 
When it comes for the code to be reused, the new data is often incomplete and this can cause difficult to diagnose bugs.
...
We can write tests that expect an error using the function `expect_error` to make sure we have handled these cases correctly.
...
All the tests in a package can then be run with the function `test` from the testthat package but also by running `check` from the devtools package [@devtools] which also checks other aspects of the package.
