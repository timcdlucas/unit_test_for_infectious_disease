...
author:
  ...
  - name: Timothy M Pollington
    ...
    affiliation: MathSys CDT, Big Data Institute.
...
abstract: |
  ...
  They help guide health policy that can affect millions of people. 
...
expect_true(all(c("a", "b", "c") %in% pop1[1,])) # expect all 3 strains @ t = 1
...
The probability of a pair not being infected by the same person is $1-1/n$ (given that the first person is infected by person $a$, the probability that the second person is infected by $a$ is just $1/n$).
For the test to fail, every pair would have to not be infected by the same person giving $\left(1-1/n\right) ^ {n \choose 2}$.
...
This strategy works best when we have a specific bug that occurred relatively frequently (perhaps 1:10 simulations).
...
However, bugs often occur when parameters are at the edges of their ranges or special values.
...
Code that relies on the returned object being a matrix would fail in the vector edge case.
...
Here again there is probably no solution other than a check at the beginning of the function that returns an error if `strain > N`.

```{r, edge2, eval = FALSE}
pop <- initialisePop(10, 2, strain = 11) 
...
```{r, wrong1, eval = FALSE}
expect_error(pop <- fullSim(4, 1:100))
```
...
All the tests in a package can then be run with the function `test()` from the $\texttt{testthat}$ package but also by running `check()` from the devtools package [@devtools] which also checks other aspects of the package.
...
Unit tests within a Julia package live in the file `test/runtests.jl` and the tests can be run with `Pkg.test()`.
...
This file can also be setup with the $\texttt{usethis}$ function `use_travis()`.
...
Unit testing is the fundemental process by which we can reduce the number of bugs in our code.
