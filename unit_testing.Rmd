...
abstract: |
  ...
  Yet a single coding bug could bias results, leading to incorrect conclusions and wrong actions that could cause avoidable harm.
...
Outside of epidemiology, a paper that informed austerity policies globally was found to be fatally flawed [@herndon2014does] while in engineering bugs have caused the Mars Climate orbiter and the Mariner 1 spacecraft, to become lost or destroyed [@nasa; @board1999mars].
...
As the mathematical methods depend on numerical solutions rather than analytical pen-and-paper methods, it becomes increasingly difficult to tell if a bug is present based on model outputs alone.
...
For the square root function $\sqrt{x}$ (`sqrt(x)` in \emph{R} [@R]), we could write a test that runs the function for the number 4 i.e. `sqrt(x = 4)` and compares it to the correct answer i.e. 2.
...
```{r modelexample, echo=FALSE, out.width="100%", fig.cap="The 3-pathogen system with arrows showing the possible transitions at every time step."}
...
```{r, correctplots, echo = FALSE, fig.cap = 'The correct behaviour with the proportion of each pathogen drifting to either superiority or extinction. Each pathogen is a different line.', out.width = "75%"}
