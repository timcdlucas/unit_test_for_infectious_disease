...
There are a number of ways that unit testing can be incorporated into the programming workflow.
The first and most obvious is that as you write code, you should write tests that confirm that the code does what you think it does.
The second, is that these same tests should then be regularly run as you develop new code.
...
Thirdly, whenever a bug is found in the code, a test should be written that captures the bug. 
Then if the bug re-emerges it will be caught.
Finally, you can write tests before you write code---also known as test-driven development.
...
Consider a multistrain system, with a population of $N$ individuals who each get reinfected exactly once per time step.
...
Setting `replace = TRUE` fixes the code and when we plot the proportion of each strain (Figure \@ref(fig:correctplots)) we now see the correct behaviour (one strain drifting to fixation.
...
This same function is then used in both the larger model code and in the unit test itself.
...
Our function `update_pop` performs one time step of the simulation, so we expect one additional row to be populated.
