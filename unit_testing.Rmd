...
For example, if we set up a very small population with only one strain, we know that both the first and second time step should only contain strain a.


```{r, test_easy, results = 'hide'}
pop <- initialise_pop(3, 2, strain = 1) 
pop2 <- update_pop(pop, 3, 2)
expect_true(all(pop2 == 'a'))
```
...
In the last test we have tested the exact value we expect the first row to take.
...
pop4 <- initialise_pop(3, 2, strain = 2) 
expect_equal(pop4[1, ], c('a', 'b', 'a'))
