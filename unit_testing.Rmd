...
library(ggplot2)
library(testthat)
knitr::opts_chunk$set(cache = FALSE, fig.width = 8, fig.height = 5, out.extra = '')
```
...
(apply(I, 1, function(x) table(factor(x, levels = c("a", "b", "c")))) / N) %>% 
  t %>% 
  matplot(type = 'l', xlab = 'Time', ylab = 'Proportion')
```
...
initialise_pop <- function(N, t){
  I <- matrix(NA, ncol = N, nrow = t)
  I[1, ] <- rep(c("a", "b", "c"), N / 3)
  return(I)
}

update_pop <- function(x, N, t){
  x[t, ] <- sample(x[t - 1, ], N, replace = TRUE)
  return(x)
}
```

We can then write tests for each function.
After initialising our population, we expect the matrix to be an $N \times t$ matrix with only the first row populated.
We also expect each of the strains a, b and c to occur in the first row.
Our function update pop performs one time step of the simulation, so we expect one additional row to be populated.

```{r, test1, results = 'hide'}
pop <- initialise_pop(12, 20) 
expect_equal(dim(pop), c(20, 12))
expect_true(all(is.na(pop[-1, ])))

populated_rows1 <- sum(apply(pop, 1, function(x) !all(is.na(x))))
pop2 <- update_pop(pop, 12, 2)
populated_rows2 <- sum(apply(pop2, 1, function(x) !all(is.na(x))))
expect_equal(populated_rows2 - populated_rows1, 1)
```
