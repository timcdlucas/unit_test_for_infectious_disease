...
Furthermore, checking models in an _ad hoc_ way is biased as unexpected results trigger careful checks of the code while models that show expected behaviour are assumed bug-free.
...
popBroken <- initialisePopBroken(n_steps = 2, N = 3, pathogens = 3) 
...
set.seed(11121955)
manySims <- replicate(500, fullSim(n_steps = 20, N = 40, pathogens = 3), 
                      simplify = FALSE)
...
In this section we find frailties in the code that are conceptually different to a bug; the model as specified is already implemented correctly.
Instead we are making the code more user-friendly and user-safe to protect against mistakes if the code is reused by other scientists or by the developer in the future once they have forgotten the specifics of the code.
\newline
\newline
...
expect_error(initialisePop(n_steps = 10, N = 4, pathogens = "three"))
```
...
The code behaves qualitatively differently for `n_steps = 1` than for `n_steps = 2` because the loop in \@ref(combinesimplefunc) runs from 2 to `n_steps`.
When `n_steps` is 1, the population is initialised with 1 row but the code still tries to run a second iteration and tries to put the results in the second row of the population matrix even though that matrix only has 1 row.
...
In \@ref(edge3) we use an if statement to explicitly handle the special case of `n_steps = 1` and our test now passes.
\newline
