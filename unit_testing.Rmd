...
update_pop <- function(x, N, i){
  ...
  new_i <- new_infection_status(x[i - 1, ])
...
In the case of a population of two people, with two strains, the only possible outcomes in the second iteration are (a, a), (a, b), (b, a) and (b, b).


```{r, test_stoch_fewvalues, results = 'hide'}
pop <- initialise_pop(2, 2, strain = 2) 
# Run first update 300 times.
# Collapse each population into a single string to make comparisons easier.
many_pops <- replicate(300, paste0(update_pop(pop, 2, 2)[2, ], collapse = ''))

expect_true(all(many_pops %in% c('aa', 'ab', 'ba', 'bb')))
