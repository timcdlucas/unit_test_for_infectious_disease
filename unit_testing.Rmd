...
However, this simply reinforces that time should be taken to test and document code when it is first written so that it is already well tested when it is needed under short deadlines.
...
manySims <- replicate(500, 
                      fullSim(n_steps = 20, N = 40, pathogens = 3), 
                      simplify = FALSE)
...
Instead we are making the code more user-friendly and user-safe to protect against mistakes if the code is reused by other scientists or by the developer in the future.
\newline
\newline
