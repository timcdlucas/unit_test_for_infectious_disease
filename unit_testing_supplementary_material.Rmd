...
\newline
```{r, test_complex, ref = "testomplex", codecap = "Test logical structure of the code rather than the specific numerical values.", results = 'hide'}
...
For example, `nextEvent` performs stochastic and deterministic operations.
The function is copied below (\@ref(splitdeterstoch1)) for reference.
\newline
```{r, split_deter_stoch1, ref = "splitdeterstoch1", codecap = "Isolation of the determistic and stochastic elements"}
nextEvent <- function(sim, pf1, pf){
  # Select whether the event is a transmission
  #   or recovery event.
  if (runif(1) < (pf1 / pf)) {
...
It then performs that event, althought we note we have already pulled much of the code out into the two functions `infection()` and `recovery()`.
However, we can split this code into two functions, one (`nextEvent()`) completely deterministic, and one very small function (`chooseEvent()`) that carries out the stochastic component.
The deterministic function an be tested using the guidelines above while in the following sections we will look at methods to test the stochastic function.
...
Given that the `runif` function is a built in R function that is well tested, we would not have to test it.
However, for demonstration purposes we have split the function as below show that we can subsequently demonstrate how to test stochastic functions.
```{r, split_deter_stoch2, ref = "splitdeterstoch2", codecap = "Isolation of the determistic and stochastic elements"}
...
For the function `chooseEvent()` this is trivially true; for anly valid values of `pf1` and `pf` it should return either `TRUE` or `FALSE`.
\newline
...
However, if we run the function thousands times, and `pf1` is approximately half of `pf` (i.e. infection and recovery events are equally likely) it is almost certain that we will get at least one of each logical value.
\newline
...
So we can run the simulation a number of times and check that it doesn't occur.
However, without a preknown bug, it is impossible to know how many runs of the simulation we would need to run before finding the bug, so in this example we will simply run 100.
\newline
...
\newline
\newline
We can first check that the `calcRates()` function  sensibly handles the user inputting a string instead of an number for the number of $\beta$ parameter.
...
The easiest way to protect against this would be to use defensive programming and check the types of the arguments.
We demonstrate this is \@ref(wrong1c).
...
\newline
```{r, edge2, ref = "edge2", codecap = "", eval = FALSE}
> sim <- initialiseSim(100, 4, 0)
...
In general, requesting a simulation with a recovery rate of zero is not unmeaningful; this is not an objectively wrong use of the function. 
...
It might be appropriate to simply declare that this parameter value is not allowed; setting $\gamma = 0$ means this is no longer an SIR model but is instead an SI model.
This should be stated in the documentation and the function should throw an error.
This is what we will do here (\@ref(edge3)).
We change our defensive lines in `calcRates` so that `gamma` must be greater than 0 rather than great or equal than zero.
...
\newline
```{r, edge4, ref = "edge4", codecap = "", eval = FALSE}
sim <- initialiseSim(100, 4, 0)
